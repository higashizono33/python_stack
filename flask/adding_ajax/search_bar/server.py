...
def process_message():
    if request.method == 'POST':
        if len(request.form['message']) < 5:
            flash("Please enter at least 5 charactors in your message")
        if not '_flashes' in session.keys():
            mysql = connectToMySQL('private_wall')
...
def search():
    ...
    if request.args.get('name') == '':
        results = []
    return render_template("partial/search.html", users = results) # render a template which uses the results
